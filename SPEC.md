...
(23 specific actions across 6 strategies — VERIFIED against the PDF; ships a department key mapping
...
|---|---|---|
| `tcm` | snapshot temperature, tiles in **°C** (MEASURED 2026-08-23; the quickstart README says °F and is wrong — see docs/api_findings.md §1) | — |
...
| `persistence` | **longest continuous run** of such hours. **BROKEN at `filter_type=4` — saturates at 8.0 regardless of threshold. Correct at `filter_type=3`.** (MEASURED) | threshold + direction |
...
- **Area:** docs claim 10 mi² (Basic) / 50 mi² (Premium). MEASURED on the
  Hackathon plan: **1,053 mi² / 272,917 tiles accepted in one call.** The whole
  of Phoenix fits in a single request.
- **Credits: 4,220 per heatmap call, FLAT** — independent of tile count. A
  420-tile call costs the same as a 272,917-tile call. Never make small calls.
...
1. **Unit chain.** Phoenix plan states thresholds in **°F**. API `threshold` is
   **°C**. `tcm` tiles come back in **°C** — CORRECTED, measured. Convert exactly once, in
