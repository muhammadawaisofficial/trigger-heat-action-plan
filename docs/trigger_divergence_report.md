...
The committed cache is 87 MB covering 258 API responses. **All thermal data comes from the FortyGuard Temperature API; no external weather or temperature service is used anywhere in the pipeline.**
...
*Generated by `make_report.py` on 2026-08-29. Every figure is read or computed from the pipeline's result files; none is transcribed by hand.*
