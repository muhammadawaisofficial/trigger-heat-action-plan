...
TRIGGER compiles heat plans, not Phoenix's heat plan. Everything city-specific lives in one JSON profile, and nothing city-specific is baked into the pipeline. Adding a city is five files and one environment variable. No code changes.

This has been done once already, for New York, which is why the claim is demonstrated rather than asserted: `data/cities/nyc.json` is the worked example every step below refers to.
...
- **One box, not many.** Credits are flat per call regardless of area, so a single box covering the whole city is strictly better than several small ones.
...
The pipeline. `evaluate.py`, `aggregate.py`, `diverge.py`, `charts.py`, and every page of the app read the active profile through `study.py` and never name a city. That is the portability claim, and the New York run is what tests it: the same code, unmodified, produced 2,453,713 people across 51 community districts.
...
- **A hardcoded cache label** meant every city's responses claimed to be Phoenix's. Harmless to correctness, since the cache key hashes the request payload, but actively misleading to anyone auditing the cache.
...
A portability claim that has only ever been run on one city is a hypothesis. This one has been run on two.
